...
export default function KelolaPengguna() {
  ...
  const handleInvite = async () => {
    ...
    try {
      // Endpoint admin POST /api/user (khusus admin) — register publik tidak menerima role
      await api.create("user", { email, password: 'password123', full_name: email.split('@')[0], role: selectedRole });
  ...
  const handleAddUser = async () => {
    ...
    setAddSaving(true);
    try {
      await api.create("user", { email: addEmail, password: 'password123', full_name: addEmail.split('@')[0], role: 'biro_pengusul' });
      toast.success('User berhasil dibuat dengan password default: password123');
