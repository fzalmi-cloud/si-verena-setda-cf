...
export default function Pemeriksaan() {
  ...
  const handleAutoVerifikasi = async () => {
    ...
    const dokumenUrl = dokumenList[0]?.file_url ? getFileUrl(dokumenList[0].file_url) : null;
    const refUrls = fileReferensi.map(f => getFileUrl(f.file_url)).filter(Boolean);
  ...
  const upsertSkor = async (extraData = {}) => {
    ...
    try {
      const res = await api.list('skor', { nama_biro: selectedBiro, tahun });
      const existingList = res?.data || (Array.isArray(res) ? res : []);
      if (existingList.length > 0) {
        await api.update('skordokumen', existingList[0].id, scoreData);
      } else {
        await api.create('skordokumen', scoreData);
      }
    } catch (err) {
      console.error('upsertSkor error:', err);
    }
  };
